...
class RibXYZGenerator:
    ...
    def export_xyz(self, out_dir: str):
        ...
        Enforces uniform point distribution via linear resampling.
        """
        os.makedirs(out_dir, exist_ok=True)
        
        # Define target point count per section to ensure loft stability
        TARGET_POINTS = 128 
        TOLERANCE = 1e-4
        ...
        for i in range(self.N):
            ...
            # 2. Generate initial 2D geometry
            raw_pts_2d = generate_rib_2d(
                RH=p["RH"],
                LA=p["LA"],
                TW=p["TW"],
                BW=p["BW"],
            ...
            # 3. Uniform Resampling Logic
            # Ensures even point distribution even on straight lines
            pts_np = np.array(raw_pts_2d)
            # Calculate Euclidean distance between consecutive points
            dists = np.sqrt(np.sum(np.diff(pts_np, axis=0)**2, axis=1))
            # Create cumulative path length array
            cumulative_dist = np.concatenate(([0], np.cumsum(dists)))
            total_len = cumulative_dist[-1]
            
            if total_len > 1e-6:
                # Create uniformly spaced sample locations
                sample_locations = np.linspace(0, total_len, TARGET_POINTS)
                # Linearly interpolate new coordinates
                resampled_x = np.interp(sample_locations, cumulative_dist, pts_np[:, 0])
                resampled_y = np.interp(sample_locations, cumulative_dist, pts_np[:, 1])
                pts_2d = np.column_stack((resampled_x, resampled_y)).tolist()
            else:
                pts_2d = raw_pts_2d

            # 4. Map 2D points to 3D cylindrical space
            pts_3d = map_2d_to_3d(
                pts_2d,
                p["R"],
                p["FA"],
        ...
        print(f"[XYZ] Exported {self.N} sections ({TARGET_POINTS} pts/sec) to: {out_dir}")
