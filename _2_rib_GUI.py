...
class RibGUI(tk.Frame):
    ...
    def build_ui(self):
        ...
        # --- Action Bar at the Bottom of Left Panel ---
        ...
        # 2. STEP Generation Button (Newly Added)
        ttk.Button(
            action_bar,
            text="💎 GENERATE 3D STEP MODEL",
            style="Main.TButton",
            command=self.generate_rib_step_handler
        ).pack(side="bottom", fill="x", pady=5)

        # 3. XYZ & Preview Generation Button
        ttk.Button(
            action_bar,
    ...
    def generate_rib_step_handler(self):
        try:
            from _2a_rib_path import ensure_dirs
            from _2e_STEP_Output import export_rib_step
            ...
            if not any(
                fn.startswith(f"1_{mn}_RibSection")
                and fn.endswith(".txt")
                for fn in os.listdir(XYZ_ROOT_DIR)
            ):
                self.status_var.set("❌ No Rib XYZ found. Please generate Rib XYZ first.")
                return

            self.status_var.set(f"⏳ OCP: Generating 3D STEP for {mn}...")
            self.update_idletasks()

            success, result = export_rib_step(mn, fan_params=fan_params)

            if success:
                self.status_var.set(f"✅ STEP SAVED: {os.path.basename(result)}")
                # os.startfile(os.path.dirname(result))
            else:
                self.status_var.set(f"❌ STEP ERROR: {result}")

        except Exception as e:
            traceback.print_exc()
            self.status_var.set(f"❌ CRITICAL: {str(e)}")
